...
export const MovieCard: React.FC<MovieCardProps> = ({ movie, onWatch, onQuickShare }) => {
  ...
  return (
    ...
    >
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
          ...
          {country && (
            <span
              ...
              className="px-1.5 py-0.5 rounded bg-black/80 backdrop-blur-sm border border-sky-500/30 text-[10px] font-bold text-sky-200 flex items-center gap-1 w-max"
            >
      ...
      {/* Movie Details */}
      <div className="p-2.5 xs:p-3 sm:p-4 flex flex-col flex-grow justify-between gap-2">
        <div>
          <h3 className="font-bold text-xs xs:text-sm text-[#F9F9F9] line-clamp-2 group-hover:text-[#E50914] transition-colors leading-snug">
            {movie.title}
          </h3>
          ...
          {movie.genres && movie.genres.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {movie.genres.slice(0, 2).map((g) => (
                <span
                  key={g}
                  className="text-[9px] xs:text-[10px] font-medium text-[#A1A1AA] bg-[#14141C] border border-[#20202C] px-1.5 py-0.5 rounded truncate max-w-[90px]"
                >
                  {g}
                </span>
              ))}
        ...
        {/* Action Bottom Row */}
        <div className="pt-2 border-t border-[#181822] flex items-center gap-1.5 xs:gap-2">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onWatch(movie.slug);
            }}
            className="flex-1 bg-[#E50914] hover:bg-[#F40612] active:bg-[#B80710] text-white text-center py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] xs:text-xs font-bold uppercase tracking-wider transition shadow-sm shadow-[#E50914]/20 flex items-center justify-center gap-1 sm:gap-1.5"
          >
          ...
          <button
            ...
            className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl bg-[#14141C] hover:bg-[#1C1C26] active:bg-[#E50914]/20 border border-[#20202C] text-[#A1A1AA] hover:text-[#F9F9F9] transition shrink-0"
          >
            {copied ? (
